# ADR-042: Airflow 3, Single-Engine
...
Rejected: keep an Airflow 2 compatibility engine; the stack has no installed
base and the duplicate topology would exist only for transition.

Rejected: wait for the community workflow service; the stack ships no DAGs,
so the middleware engine can move independently.
...
- The stack tracks the current Airflow major with one component and no
  transition machinery.
- Signing keys stay stable across Flux reconciles and remain CLI-owned.
- The OSDU workflow service can lag Airflow majors, leaving its Airflow
  integration dormant until its images support the deployed engine.

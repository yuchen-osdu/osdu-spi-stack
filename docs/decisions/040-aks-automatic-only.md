# ADR-040: AKS Automatic as the Only Cluster Topology

## Context

AKS Automatic on Kubernetes 1.36 supports the mutating webhooks required by
the middleware operators (ADR-031). The temporary Base plus Node
...
with the Kubernetes 1.36 constraint from ADR-031.

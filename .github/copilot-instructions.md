...
- AKS Automatic 1.36 by default; Base + Node Autoprovisioning is selected with
  `--aks-mode base` and preserved per environment (ADR-031)
...
- Local auth disabled on Gremlin and Service Bus; Cosmos SQL keeps its key path
  until Partition supports the Cosmos MSI client (ADR-033)
- Application Insights is optional/default-off, persisted per environment, and
  wired to all services in both modes (ADR-023, ADR-028)
- Record-ingestion data plane enabled: system-cosmos secrets, per-partition record
  blob container, Elasticsearch TLS (ADR-029)
- The ingress profile is the only Gateway owner (ADR-034)
- Identity is projected per caller; access requires explicit Entitlements
  membership seeding (ADR-035)
- Managed Istio revision is resolved at deploy time, never hardcoded (ADR-032)
- Service images default to the public GHCR SPI fleet, pinned by digest (ADR-030)
